...
# soom_hash
RUN git clone https://github.com/soom-4th-blockchain/soom_hash
RUN cd soom_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} soom
RUN useradd -u ${USER_ID} -g soom -s /bin/bash -m -d /soom-core soom
...
RUN mkdir /soom-src && \
  ...
  chown $USER_ID:$GROUP_ID /soom-src && \
