...
class SporkTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Force finish gwsync node as otherwise it will never send out headers to other peers
        wait_to_sync(self.nodes[1], fast_gwsync=True)

        # Generate one block to kick off gateway sync, which also starts sporks syncing for node2
