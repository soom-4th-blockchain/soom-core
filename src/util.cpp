...
const char * const BITCOIN_CONF_FILENAME = "soom.conf";
const char * const BITCOIN_PID_FILENAME = "soomd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "soom" is a composite category enabling all Soom-related debug output
                if(ptrCategory->count(std::string("soom"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SoomCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SoomCore
    // Mac: ~/Library/Application Support/SoomCore
    // Unix: ~/.soomcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SoomCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SoomCore";
#else
    // Unix
    return pathRet / ".soomcore";
#endif
#endif
}
...
boost::filesystem::path GetGatewayConfigFile()
{
...
//#/ start jhhong createConf 180801
void createConf(const std::string& confPath)       //Automatic soom.conf generation
{
    srand(time(NULL));

    std::ofstream pConf;
    pConf.open(GetConfigFile(confPath).generic_string().c_str());
    const char* nodes =  "\nlisten=1"
                         "\ndaemon=1"
                         "\nserver=1";
...
//#/ start jhhong createConf 180801
		createConf(confPath);
//@/ start jhhong createConf
        // Create empty soom.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override soom.conf
...
{
    std::string strText = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS), _(COPYRIGHT_HOLDERS_SUBSTITUTION));
