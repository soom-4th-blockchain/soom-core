...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    // It's allowed to set addr to 0, which will put the GW into PoSe-banned state and require a ProUpServTx to be issues later
    ...
    if (!ptx.collateralOutpoint.hash.IsNull()) {
        Coin coin;
        if (!GetUTXOCoin(ptx.collateralOutpoint, coin) || coin.out.nValue != COLLATERAL_COINS) {
        ...
        if (tx.vout[ptx.collateralOutpoint.n].nValue != COLLATERAL_COINS) {
            return state.DoS(10, false, REJECT_INVALID, "bad-protx-collateral");
        }
    ...
    if (pindexPrev) {
        auto gwList = deterministicGWManager->GetListForBlock(pindexPrev->GetBlockHash());

        // only allow reusing of addresses when it's for the same collateral (which replaces the old GW)
        if (gwList.HasUniqueProperty(ptx.addr) && gwList.GetUniquePropertyGW(ptx.addr)->collateralOutpoint != collateralOutpoint) {
            return state.DoS(10, false, REJECT_DUPLICATE, "bad-protx-dup-addr");
        }

        // never allow duplicate keys, even if this ProTx would replace an existing GW
        if (gwList.HasUniqueProperty(ptx.keyIDOwner) || gwList.HasUniqueProperty(ptx.pubKeyOperator)) {
            return state.DoS(10, false, REJECT_DUPLICATE, "bad-protx-dup-key");
        }

        if (!deterministicGWManager->IsDeterministicGWsSporkActive(pindexPrev->nHeight)) {
...
bool CheckProUpServTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pindexPrev) {
        auto gwList = deterministicGWManager->GetListForBlock(pindexPrev->GetBlockHash());
        auto gw = gwList.GetGW(ptx.proTxHash);
        if (!gw) {
            return state.DoS(100, false, REJECT_INVALID, "bad-protx-hash");
        }

        // don't allow updating to addresses already used by other GWs
        if (gwList.HasUniqueProperty(ptx.addr) && gwList.GetUniquePropertyGW(ptx.addr)->proTxHash != ptx.proTxHash) {
            return state.DoS(10, false, REJECT_DUPLICATE, "bad-protx-dup-addr");
        }

        if (ptx.scriptOperatorPayout != CScript()) {
            if (gw->nOperatorReward == 0) {
        ...
        // we can only check the signature if pindexPrev != NULL and the GW is known
        if (!CheckInputsHash(tx, ptx, state)) {
            return false;
        }
        if (!CheckHashSig(ptx, gw->pdgwState->pubKeyOperator, state)) {
            return false;
        }
    }
...
bool CheckProUpRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pindexPrev) {
        auto gwList = deterministicGWManager->GetListForBlock(pindexPrev->GetBlockHash());
        auto dgw = gwList.GetGW(ptx.proTxHash);
        if (!dgw) {
            return state.DoS(100, false, REJECT_INVALID, "bad-protx-hash");
        }

        // don't allow reuse of payee key for other keys (don't allow people to put the payee key onto an online server)
        if (payoutDest == CTxDestination(dgw->pdgwState->keyIDOwner) || payoutDest == CTxDestination(ptx.keyIDVoting)) {
            return state.DoS(10, false, REJECT_INVALID, "bad-protx-payee-reuse");
        }

        Coin coin;
        if (!GetUTXOCoin(dgw->collateralOutpoint, coin)) {
            // this should never happen (there would be no dgw otherwise)
        ...
        if (collateralTxDest == CTxDestination(dgw->pdgwState->keyIDOwner) || collateralTxDest == CTxDestination(ptx.keyIDVoting)) {
            return state.DoS(10, false, REJECT_INVALID, "bad-protx-collateral-reuse");
        }

        if (gwList.HasUniqueProperty(ptx.pubKeyOperator)) {
            auto otherDgw = gwList.GetUniquePropertyGW(ptx.pubKeyOperator);
            if (ptx.proTxHash != otherDgw->proTxHash) {
                return state.DoS(10, false, REJECT_DUPLICATE, "bad-protx-dup-key");
            }
        }

        if (!deterministicGWManager->IsDeterministicGWsSporkActive(pindexPrev->nHeight)) {
            if (dgw->pdgwState->keyIDOwner != ptx.keyIDVoting) {
                return state.DoS(10, false, REJECT_INVALID, "bad-protx-key-not-same");
            }
        }
        ...
        if (!CheckHashSig(ptx, dgw->pdgwState->keyIDOwner, state)) {
            return false;
        }
    }
...
bool CheckProUpRevTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pindexPrev) {
        ...
        if (!CheckInputsHash(tx, ptx, state))
            return false;
        if (!CheckHashSig(ptx, dgw->pdgwState->pubKeyOperator, state))
            return false;
    }
