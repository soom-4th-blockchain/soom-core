...
bool CheckSpecialTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    case TRANSACTION_QUORUM_COMMITMENT:
        return llgq::CheckLLGQCommitment(tx, pindexPrev, state);
    }
...
bool ProcessSpecialTxsInBlock(const CBlock& block, const CBlockIndex* pindex, CValidationState& state)
{
    ...
    if (!llgq::quorumBlockProcessor->ProcessBlock(block, pindex, state)) {
        return false;
    }

    if (!deterministicGWManager->ProcessBlock(block, pindex, state)) {
        return false;
    }

    if (!CheckCbTxMerkleRootGWList(block, pindex, state)) {
        return false;
    }
...
bool UndoSpecialTxsInBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    if (!deterministicGWManager->UndoBlock(block, pindex)) {
        return false;
    }

    if (!llgq::quorumBlockProcessor->UndoBlock(block, pindex)) {
        return false;
    }
