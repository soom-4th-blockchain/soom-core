...
/*
    At 15 signatures, 1/2 of the gateway network can be owned by
...
private:
    ...
    /// Track gateways who voted with no txlockrequest (for DOS protection)
    std::map<COutPoint, int64_t> mapGatewayOrphanVotes; ///< GW outpoint - Time
    ...
    int64_t GetAverageGatewayOrphanVoteTime();
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(mapGatewayOrphanVotes);
...
/**
 * An InstantSend transaction lock vote. Sent by a gateway in response to a
 ...
 * locked along with the gateways outpoint and signature.
 * @see CTxLockRequest
 */
class CTxLockVote
{
private:
    uint256 txHash;
    COutPoint outpoint;
    // TODO remove this member when the legacy gateway code is removed after DIP3 deployment
    COutPoint outpointGateway;
    uint256 quorumModifierHash;
    uint256 gatewayProTxHash;
    std::vector<unsigned char> vchGatewaySignature;
    // local memory only
...
public:
    ...
    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointGatewayIn, const uint256& quorumModifierHashIn, const uint256& gatewayProTxHashIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointGateway(outpointGatewayIn),
        quorumModifierHash(quorumModifierHashIn),
        gatewayProTxHash(gatewayProTxHashIn),
        vchGatewaySignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        if (deterministicGWManager->IsDeterministicGWsSporkActive()) {
            ...
            // the surrounding if. We might also remove outpointGateway as well later
            READWRITE(quorumModifierHash);
            READWRITE(gatewayProTxHash);
        }
        if (!(s.GetType() & SER_GETHASH)) {
        	READWRITE(vchGatewaySignature);
        }
    }
    ...
    COutPoint GetGatewayOutpoint() const { return outpointGateway; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapGatewayVotes; // Gateway outpoint - vote
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        READWRITE(outpoint);
        READWRITE(mapGatewayVotes);
        READWRITE(fAttacked);
    }

    bool AddVote(const CTxLockVote& vote);
    std::vector<CTxLockVote> GetVotes() const;
    bool HasGatewayVoted(const COutPoint& outpointGatewayIn) const;
    int CountVotes() const { return fAttacked ? 0 : mapGatewayVotes.size(); }
...
public:
    ...
    bool HasGatewayVoted(const COutPoint& outpointIn, const COutPoint& outpointGatewayIn);
