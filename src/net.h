...
public:
    ...
    // fConnectToGateway should be 'true' only if you want this node to allow to connect to itself
    // and/or you want it to be disconnected on CGatewayMan::ProcessGatewayConnections()
    // Unfortunately, can't make this method private like in Bitcoin,
    // because it's used in many Soom-specific places (gateway).
    CNode* ConnectNode(CAddress addrConnect, const char *pszDest = NULL, bool fConnectToGateway = false);
...
private:
    ...
    void ThreadGwbRequestConnections();
    ...
    std::thread threadGwbRequestConnections;
