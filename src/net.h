...
/** Maximum number if outgoing gateways */
static const int MAX_OUTBOUND_GATEWAY_CONNECTIONS = 20;
/** -listen default */
...
public:
    ...
    bool OpenNetworkConnection(const CAddress& addrConnect, bool fCountFailure, CSemaphoreGrant *grantOutbound = NULL, const char *strDest = NULL, bool fOneShot = false, bool fFeeler = false, bool fAddnode = false, bool fConnectToGateway = false);
    bool OpenGatewayConnection(const CAddress& addrConnect);
    ...
    bool IsGatewayOrDisconnectRequested(const CService& addr);
    ...
    bool AddPendingGateway(const CService& addr);
...
private:
    ...
    void ThreadOpenGatewayConnections();
    ...
    std::vector<CService> vPendingGateways;
    CCriticalSection cs_vPendingGateways;
    ...
    std::thread threadOpenGatewayConnections;
...
public:
    ...
    bool fSentAddr;
    // If 'true' this node will be disconnected on CGatewayMan::ProcessGatewayConnections()
    bool fGateway;
    CSemaphoreGrant grantOutbound;
    CSemaphoreGrant grantGatewayOutbound;
