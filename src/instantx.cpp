...
// step 2) Top COutPointLock::SIGNATURES_TOTAL gateways per each spent outpoint push "txlockvote" message (txlvote)
...
void CInstantSend::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::TXLOCKVOTE) { // InstantSend Transaction Lock Consensus Votes
        ...
        // Ignore any InstantSend messages until gateway list is synced
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest, CConnman& connman)
{
    ...
    // Gateways will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate, CConnman& connman)
{
    ...
    for (auto& outpointLockPair : txLockCandidate.mapOutPointLocks) {
        ...
        int nMinRequiredProtocol = std::max(MIN_INSTANTSEND_PROTO_VERSION, gwpayments.GetMinGatewayPaymentsProto());
        if(!gwnodeman.GetGatewayRank(activeGatewayInfo.outpoint, nRank, quorumModifierHash, nLockInputHeight, nMinRequiredProtocol)) {
            LogPrint("instantsend", "CInstantSend::Vote -- Can't calculate rank for gateway %s\n", activeGatewayInfo.outpoint.ToStringShort());
            continue;
        }
        ...
        if(nRank > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- Gateway not in the top %d (%d)\n", nSignaturesTotal, nRank);
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            for (const auto& hash : itVoted->second) {
                ...
                if(it2->second.HasGatewayVoted(outpointLockPair.first, activeGatewayInfo.outpoint)) {
        ...
        // Please note that activeGatewayInfo.proTxHash is only valid after spork15 activation
        CTxLockVote vote(txHash, outpointLockPair.first, activeGatewayInfo.outpoint, quorumModifierHash, activeGatewayInfo.proTxHash);
...
bool CInstantSend::ProcessNewTxLockVote(CNode* pfrom, const CTxLockVote& vote, CConnman& connman)
{
    ...
    if(!vote.IsValid(pfrom, connman)) {
        // could be because of missing GW
    ...
    if(it == mapTxLockCandidates.end() || !it->second.txLockRequest) {
        ...
        LogPrint("instantsend", "CInstantSend::%s -- Orphan vote: txid=%s  gateway=%s %s\n",
                __func__, txHash.ToString(), vote.GetGatewayOutpoint().ToStringShort(), fInserted ? "new" : "seen");
        ...
        int nGatewayOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        auto itGwOV = mapGatewayOrphanVotes.find(vote.GetGatewayOutpoint());
        if(itGwOV == mapGatewayOrphanVotes.end()) {
            mapGatewayOrphanVotes.emplace(vote.GetGatewayOutpoint(), nGatewayOrphanExpireTime);
        } else {
            if(itGwOV->second > GetTime() && itGwOV->second > GetAverageGatewayOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::%s -- gateway is spamming orphan Transaction Lock Votes: txid=%s  gateway=%s\n",
                        __func__, txHash.ToString(), vote.GetGatewayOutpoint().ToStringShort());
                // Misbehaving(pfrom->id, 1);
                return false;
            }
            // not spamming, refresh
            itGwOV->second = nGatewayOrphanExpireTime;
        }
    ...
    if(it1 != mapVotedOutpoints.end()) {
        for (const auto& hash : it1->second) {
            if(hash != txHash) {
                ...
                // let's see if it was the same gateway who voted on this outpoint
                // for another tx lock request
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2 !=mapTxLockCandidates.end() && it2->second.HasGatewayVoted(vote.GetOutpoint(), vote.GetGatewayOutpoint())) {
                    // yes, it was the same gateway
                    LogPrintf("CInstantSend::%s -- gateway sent conflicting votes! %s\n", __func__, vote.GetGatewayOutpoint().ToStringShort());
                    ...
                    // apply maximum PoSe ban score to this gateway i.e. PoSe-ban it instantly
                    gwnodeman.PoSeBan(vote.GetGatewayOutpoint());
                    // NOTE: This vote must be relayed further to let all other nodes know about such
                    // misbehaviour of this gateway. This way they should also be able to construct
                    // conflicting lock and PoSe-ban this gateway.
                }
            }
        }
        // store all votes, regardless of them being sent by malicious gateway or not
...
bool CInstantSend::ResolveConflicts(const CTxLockCandidate& txLockCandidate)
{
    ...
    for (const auto& txin : txLockCandidate.txLockRequest.tx->vin) {
        ...
        if(GetLockedOutPointTxHash(txin.prevout, hashConflicting) && txHash != hashConflicting) {
            ...
            // this means that majority of GWs in the quorum for this specific tx input are malicious!
            std::map<uint256, CTxLockCandidate>::iterator itLockCandidate = mapTxLockCandidates.find(txHash);
            ...
            //       (not a big issue since we already PoSe ban malicious gateways
            //        and they won't be able to spam)
            // TODO: ban all malicious gateways permanently, do not accept anything from them, ever
...
int64_t CInstantSend::GetAverageGatewayOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapGatewayOrphanVotes is empty
    if(mapGatewayOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapGatewayOrphanVotes.begin();
    int64_t total = 0;
    for (const auto& pair : mapGatewayOrphanVotes) {
        total += pair.second;
    }

    return total / mapGatewayOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(nCachedBlockHeight)) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  gateway=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetGatewayOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(itOrphanVote->second.IsTimedOut()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing timed out orphan vote: txid=%s  gateway=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetGatewayOutpoint().ToStringShort());
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsFailed()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing vote for failed lock attempt: txid=%s  gateway=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetGatewayOutpoint().ToStringShort());
            mapTxLockVotes.erase(itVote++);
        } else {
            ++itVote;
    ...
    // remove timed out gateway orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itGatewayOrphan = mapGatewayOrphanVotes.begin();
    while(itGatewayOrphan != mapGatewayOrphanVotes.end()) {
        if(itGatewayOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing timed out orphan gateway vote: gateway=%s\n",
                    itGatewayOrphan->first.ToStringShort());
            mapGatewayOrphanVotes.erase(itGatewayOrphan++);
        } else {
            ++itGatewayOrphan;
        }
    }
...
{
    if(!gwnodeman.Has(outpointGateway)) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown gateway %s\n", outpointGateway.ToStringShort());
        gwnodeman.AskForGW(pnode, outpointGateway, connman);
        return false;
    }

    // Verify that gatewayProTxHash belongs to the same GW referred by the collateral
    // Only v13 nodes will send us locks with this field set, and only after spork15 activation
    if (!gatewayProTxHash.IsNull()) {
        gateway_info_t gwInfo;
        if (!gwnodeman.GetGatewayInfo(gatewayProTxHash, gwInfo) || gwInfo.outpoint != outpointGateway) {
            LogPrint("instantsend", "CTxLockVote::IsValid -- invalid gatewayProTxHash %s\n", gatewayProTxHash.ToString());
            return false;
        }
    } else if (deterministicGWManager->IsDeterministicGWsSporkActive()) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- missing gatewayProTxHash while DIP3 is active\n");
        return false;
    }
    ...
    int nMinRequiredProtocol = std::max(MIN_INSTANTSEND_PROTO_VERSION, gwpayments.GetMinGatewayPaymentsProto());
    if(!gwnodeman.GetGatewayRank(outpointGateway, nRank, expectedQuorumModifierHash, nLockInputHeight, nMinRequiredProtocol)) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Can't calculate rank for gateway %s\n", outpointGateway.ToStringShort());
        return false;
    }
    ...
    LogPrint("instantsend", "CTxLockVote::IsValid -- Gateway %s, rank=%d\n", outpointGateway.ToStringShort(), nRank);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(nRank > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Gateway %s is not in the top %d (%d), vote hash=%s\n",
                outpointGateway.ToStringShort(), nSignaturesTotal, nRank, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    if(!gwnodeman.GetGatewayInfo(outpointGateway, infoGw)) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown Gateway: gateway=%s\n", outpointGateway.ToString());
        return false;
    }
    ...
    if (deterministicGWManager->IsDeterministicGWsSporkActive()) {
        ...
        CBLSSignature sig;
        sig.SetBuf(vchGatewaySignature);
        if (!sig.IsValid() || !sig.VerifyInsecure(infoGw.blsPubKeyOperator, hash)) {
        ...
        if (!CHashSigner::VerifyHash(hash, infoGw.legacyKeyIDOperator, vchGatewaySignature, strError)) {
            // could be a signature in old format
            std::string strMessage = txHash.ToString() + outpoint.ToStringShort();
            if(!CMessageSigner::VerifyMessage(infoGw.legacyKeyIDOperator, vchGatewaySignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if (deterministicGWManager->IsDeterministicGWsSporkActive()) {
        ...
        CBLSSignature sig = activeGatewayInfo.blsKeyOperator->Sign(hash);
        if (!sig.IsValid()) {
            return false;
        }
        sig.GetBuf(vchGatewaySignature);
    } else if (sporkManager.IsSporkActive(SPORK_6_NEW_SIGS)) {
        uint256 hash = GetSignatureHash();

        if(!CHashSigner::SignHash(hash, activeGatewayInfo.legacyKeyOperator, vchGatewaySignature)) {
            LogPrintf("CTxLockVote::Sign -- SignHash() failed\n");
            return false;
        }

        if (!CHashSigner::VerifyHash(hash, activeGatewayInfo.legacyKeyIDOperator, vchGatewaySignature, strError)) {
        ...
        if(!CMessageSigner::SignMessage(strMessage, vchGatewaySignature, activeGatewayInfo.legacyKeyOperator)) {
            LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
            return false;
        }

        if(!CMessageSigner::VerifyMessage(activeGatewayInfo.legacyKeyIDOperator, vchGatewaySignature, strMessage, strError)) {
...
{
    return mapGatewayVotes.emplace(vote.GetGatewayOutpoint(), vote).second;
}

std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    std::vector<CTxLockVote> vRet;
    for (const auto& pair : mapGatewayVotes) {
        vRet.push_back(pair.second);
    }
    return vRet;
}

bool COutPointLock::HasGatewayVoted(const COutPoint& outpointGatewayIn) const
{
    return mapGatewayVotes.count(outpointGatewayIn);
}
...
bool CTxLockCandidate::HasGatewayVoted(const COutPoint& outpointIn, const COutPoint& outpointGatewayIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasGatewayVoted(outpointGatewayIn);
}
