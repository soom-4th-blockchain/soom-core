...
// step 2) Top COutPointLock::SIGNATURES_TOTAL gateways per each spent outpoint push "txvote" message
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate, CConnman& connman)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int nRank;
        if(!gwnodeman.GetGatewayRank(activeGateway.outpoint, nRank, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION)) {
...
bool CInstantSend::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote, CConnman& connman)
{
    ...
    if(it == mapTxLockCandidates.end() || !it->second.txLockRequest) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  gateway=%s new\n",
                    txHash.ToString(), vote.GetGatewayOutpoint().ToStringShort());
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  gateway=%s seen\n",
                    txHash.ToString(), vote.GetGatewayOutpoint().ToStringShort());
        }
        ...
        if(!mapGatewayOrphanVotes.count(vote.GetGatewayOutpoint())) {
            mapGatewayOrphanVotes[vote.GetGatewayOutpoint()] = nGatewayOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapGatewayOrphanVotes[vote.GetGatewayOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageGatewayOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- gateway is spamming orphan Transaction Lock Votes: txid=%s  gateway=%s\n",
                        txHash.ToString(), vote.GetGatewayOutpoint().ToStringShort());
        ...
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                if(it2 !=mapTxLockCandidates.end() && it2->second.HasGatewayVoted(vote.GetOutpoint(), vote.GetGatewayOutpoint())) {
                    ...
                    LogPrintf("CInstantSend::ProcessTxLockVote -- gateway sent conflicting votes! %s\n", vote.GetGatewayOutpoint().ToStringShort());
...
bool CTxLockVote::IsValid(CNode* pnode, CConnman& connman) const
{
    ...
    int nRank;
    if(!gwnodeman.GetGatewayRank(outpointGateway, nRank, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION)) {
