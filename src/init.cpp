...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Soom Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Soom Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize Soom Core.
...
//@/ start jhhong fix options
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Soom Core is shutting down."));
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Soom Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Soom Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
...
//@/ start jhhong fix options
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Soom Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Soom Core to complete") << "\n";
    ...
    // ********************************************************* Step 11a: setup gateway
    fGateWay = GetBoolArg("-gateway", false);
    // TODO: gateway should have no wallet

    if((fGateWay || gatewayConfig.getCount() > -1) && fTxIndex == false) {
        return InitError("Enabling Gateway support requires turning on transaction indexing."
    ...
    if(fGateWay) {
        ...
        if(!strGateWayPrivKey.empty()) {
            if(!CMessageSigner::GetKeysFromSecret(strGateWayPrivKey, activeGateway.keyGateway, activeGateway.pubKeyGateway))
                return InitError(_("Invalid gatewayprivkey. Please see documenation."));

            LogPrintf("  pubKeyGateway: %s\n", CBitcoinAddress(activeGateway.pubKeyGateway.GetID()).ToString());
        } else {
    ...
    if(GetBoolArg("-gwconflock", true) && pwalletMain && (gatewayConfig.getCount() > 0)) {
        ...
        BOOST_FOREACH(CGatewayConfig::CGatewayEntry gwe, gatewayConfig.getEntries()) {
            ...
            outputIndex = boost::lexical_cast<unsigned int>(gwe.getOutputIndex());
    ...
    //lite mode disables all Gateway related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if(fGateWay && fLiteMode){
        return InitError("You can not start a gateway in litemode");
    }
    ...
    // force UpdatedBlockTip to initialize nCachedBlockHeight for DS, GW payments and budgets
    // but don't call it directly to prevent triggering of other listeners like zmq etc.
    // GetMainSignals().UpdatedBlockTip(chainActive.Tip());
    pgwNotificationInterface->InitializeCurrentBlockTip();
    // ********************************************************* Step 11d: start soom-gws threads

    threadGroup.create_thread(boost::bind(&ThreadCheckGatewaySync, boost::ref(*g_connman)));
