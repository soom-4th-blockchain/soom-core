...
UniValue prioritisetransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "                  (priority of a transaction is calculated: coinage * value_in_tnsoom / txsize) \n"
            "3. fee delta      (numeric, required) The fee value (in tnsoom) to add (or subtract, if negative).\n"
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "             ,...\n"
            "         ],\n"
            "         \"fee\": n,                   (numeric) difference in value between transaction inputs and outputs (in tnsoom); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
            ...
            "      \"flags\" : \"flags\"            (string) \n"
            "  },\n"
            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in tnsoom)\n"
            ...
            "  \"foundation\" : {                  (json object) foundation payee \n"
            "      \"payee\" : \"xxxx\",             (string) payee address\n"
            "      \"script\" : \"xxxx\",            (string) payee scriptPubKey\n"
            "      \"amount\": n                   (numeric) amount to pay\n"
            "  },\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Soom Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Soom Core is downloading blocks...");
    ...
    if(pblock->txoutGateway != CTxOut()) {
        ...
        gatewayObj.push_back(Pair("script", HexStr(pblock->txoutGateway.scriptPubKey.begin(), pblock->txoutGateway.scriptPubKey.end())));
        gatewayObj.push_back(Pair("amount", pblock->txoutGateway.nValue));
    }
    result.push_back(Pair("gateway", gatewayObj));
    result.push_back(Pair("gateway_payments_started", pindexPrev->nHeight + 1 > Params().GetConsensus().nGatewayPaymentsStartBlock));
    ...
    if(pindexPrev->nHeight + 1 >= Params().GetConsensus().nFoundationPaymentsStartBlock && (int64_t)pblock->vtx[0].GetValueOut() != 0)
    {
    ...
    result.push_back(Pair("foundation_payments_started", pindexPrev->nHeight + 1 > Params().GetConsensus().nFoundationPaymentsStartBlock));
