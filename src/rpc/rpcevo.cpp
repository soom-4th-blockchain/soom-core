...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The soom address to send the collateral to.\n"
        ...
        {"operatorPubKey",
            ...
            "                              It has to match the private key which is later used when operating the gateway.\n"
        },
        {"operatorReward",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The soom address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress",
            "%d. \"payoutAddress\"            (string, required) The soom address to use for gateway reward payments.\n"
        },
        {"proTxHash",
            "%d. \"proTxHash\"                (string, required) The hash of the initial ProRegTx.\n"
        },
        {"reason",
            "%d. reason                     (numeric, optional) The reason for gateway service revocation.\n"
        },
        {"votingAddress",
...
// Allows to specify Soom address or priv key. In case of Soom address, the priv key is taken from the wallet
...
void protx_register_fund_help()
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 5000 Soom\n"
...
void protx_register_help()
{
    throw std::runtime_error(
            ...
            "transaction output spendable by this wallet. It must also not be used by any other gateway.\n"
            "\nArguments:\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    CAmount collateralAmount = COLLATERAL_COINS;
    ...
    if (request.params.size() > paramIdx + 6) {
        ...
        if (!fundAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Soom address: ") + request.params[paramIdx + 6].get_str());
    }
...
void protx_update_service_help()
{
    throw std::runtime_error(
            ...
            "of a gateway.\n"
            "If this is done for a gateway that got PoSe-banned, the ProUpServTx will also revive this gateway.\n"
            "\nArguments:\n"
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    auto dgw = deterministicGWManager->GetListAtChainTip().GetGW(ptx.proTxHash);
    if (!dgw) {
        throw std::runtime_error(strprintf("gateway with proTxHash %s not found", ptx.proTxHash.ToString()));
    }

    if (keyOperator.GetPublicKey() != dgw->pdgwState->pubKeyOperator) {
    ...
    if (request.params.size() >= 5) {
        if (request.params[4].get_str().empty()) {
            ptx.scriptOperatorPayout = dgw->pdgwState->scriptOperatorPayout;
        } else {
    ...
    if (request.params.size() >= 6) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Soom address: ") + request.params[5].get_str());
        ...
        if (ptx.scriptOperatorPayout != CScript()) {
            ...
            ExtractDestination(dgw->pdgwState->scriptPayout, feeSource);
        }
    }
...
void protx_update_registrar_help()
{
    throw std::runtime_error(
            ...
            "address of the gateway specified by \"proTxHash\".\n"
            "The owner key of the gateway must be known to your wallet.\n"
            "\nArguments:\n"
            + GetHelpString(1, "proTxHash")
...
UniValue protx_update_registrar(const JSONRPCRequest& request)
{
    ...
    auto dgw = deterministicGWManager->GetListAtChainTip().GetGW(ptx.proTxHash);
    if (!dgw) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("gateway %s not found", ptx.proTxHash.ToString()));
    }
    ptx.pubKeyOperator = dgw->pdgwState->pubKeyOperator;
    ptx.keyIDVoting = dgw->pdgwState->keyIDVoting;
    ptx.scriptPayout = dgw->pdgwState->scriptPayout;
    ...
    CKey keyOwner;
    if (!pwalletMain->GetKey(dgw->pdgwState->keyIDOwner, keyOwner)) {
        throw std::runtime_error(strprintf("Private key for owner address %s not found in your wallet", CBitcoinAddress(dgw->pdgwState->keyIDOwner).ToString()));
    }
...
void protx_revoke_help()
{
    throw std::runtime_error(
            ...
            "\nCreates and sends a ProUpRevTx to the network. This will revoke the operator key of the gateway and\n"
            "put it into the PoSe-banned state. It will also set the service field of the gateway\n"
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Soom address: ") + request.params[4].get_str());
        FundSpecialTx(tx, ptx, feeSourceAddress.Get());
    } else if (dgw->pdgwState->scriptOperatorPayout != CScript()) {
        // Using funds from previousely specified operator payout address
        CTxDestination txDest;
        ExtractDestination(dgw->pdgwState->scriptOperatorPayout, txDest);
        FundSpecialTx(tx, ptx, txDest);
    } else if (dgw->pdgwState->scriptPayout != CScript()) {
        // Using funds from previousely specified gateway payout address
        CTxDestination txDest;
        ExtractDestination(dgw->pdgwState->scriptPayout, txDest);
        FundSpecialTx(tx, ptx, txDest);
    } else {
...
UniValue BuildDGWListEntry(const CDeterministicGWCPtr& dgw, bool detailed)
{
    if (!detailed) {
        return dgw->proTxHash.ToString();
    }
    ...
    int confirmations = GetUTXOConfirmations(dgw->collateralOutpoint);
    o.push_back(Pair("confirmations", confirmations));

    bool hasOwnerKey = CheckWalletOwnsKey(dgw->pdgwState->keyIDOwner);
    bool hasOperatorKey = false; //CheckWalletOwnsKey(dgw->pdgwState->keyIDOperator);
    bool hasVotingKey = CheckWalletOwnsKey(dgw->pdgwState->keyIDVoting);

    bool ownsCollateral = false;
    CTransactionRef collateralTx;
    uint256 tmpHashBlock;
    if (GetTransaction(dgw->collateralOutpoint.hash, collateralTx, Params().GetConsensus(), tmpHashBlock)) {
        ownsCollateral = CheckWalletOwnsScript(collateralTx->vout[dgw->collateralOutpoint.n].scriptPubKey);
    }
    ...
    walletObj.push_back(Pair("ownsPayeeScript", CheckWalletOwnsScript(dgw->pdgwState->scriptPayout)));
    walletObj.push_back(Pair("ownsOperatorRewardScript", CheckWalletOwnsScript(dgw->pdgwState->scriptOperatorPayout)));
...
UniValue protx_list(const JSONRPCRequest& request)
{
    ...
    if (type == "wallet") {
        ...
        CDeterministicGWList gwList = deterministicGWManager->GetListForBlock(chainActive[height]->GetBlockHash());
        gwList.ForEachGW(false, [&](const CDeterministicGWCPtr& dgw) {
            if (setOutpts.count(dgw->collateralOutpoint) ||
                CheckWalletOwnsKey(dgw->pdgwState->keyIDOwner) ||
                CheckWalletOwnsKey(dgw->pdgwState->keyIDVoting) ||
                CheckWalletOwnsScript(dgw->pdgwState->scriptPayout) ||
                CheckWalletOwnsScript(dgw->pdgwState->scriptOperatorPayout)) {
                ret.push_back(BuildDGWListEntry(dgw, detailed));
            }
        });
#endif
        ...
        gwList.ForEachGW(onlyValid, [&](const CDeterministicGWCPtr& dgw) {
            ret.push_back(BuildDGWListEntry(dgw, detailed));
        });
    } else {
...
void protx_info_help()
{
    throw std::runtime_error(
            ...
            "\nReturns detailed information about a deterministic gateway.\n"
            "\nArguments:\n"
            + GetHelpString(1, "proTxHash") +
            "\nResult:\n"
            "{                             (json object) Details about a specific deterministic gateway\n"
            "}\n"
            "\nExamples:\n"
...
UniValue protx_info(const JSONRPCRequest& request)
{
    ...
    auto dgw = gwList.GetGW(proTxHash);
    if (!dgw) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("%s not found", proTxHash.ToString()));
    }
    return BuildDGWListEntry(dgw, true);
}
...
void protx_diff_help()
{
    throw std::runtime_error(
            ...
            "\nCalculates a diff between two deterministic gateway lists. The result also contains proof data.\n"
            "\nArguments:\n"
...
UniValue protx_diff(const JSONRPCRequest& request)
{
    ...
    CSimplifiedGWListDiff gwListDiff;
    std::string strError;
    if (!BuildSimplifiedGWListDiff(baseBlockHash, blockHash, gwListDiff, strError)) {
        throw std::runtime_error(strError);
    }

    UniValue ret;
    gwListDiff.ToJson(ret);
    return ret;
}
...
#endif
            "  diff              - Calculate a diff and a proof between two gateway lists\n"
    );
}
