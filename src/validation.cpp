...
const std::string strMessageMagic = "SoomCoin Signed Message:\n";
...
//#/ hcdo check banned tx  --------------------------------------------------------------------------------------------
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Soom
    ...
    for (int i = 0; i < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; i++) {
        ...
        if (vbinfo.check_gw_protocol && state == THRESHOLD_STARTED && !fAssumeGatewayIsUpgraded) {
            if (deterministicGWManager->IsDeterministicGWsSporkActive()) {
                ...
                if (!gwpayments.GetBlockTxOuts(pindexPrev->nHeight + 1, 0, voutGatewayPayments)) {
                    // no votes for this block
                    continue;
                }
                bool gwKnown = false;
                for (const auto& txout : voutGatewayPayments) {
                    if (gwnodeman.GetGatewayInfo(txout.scriptPubKey, gwInfo)) {
                        gwKnown = true;
                        break;
                    }
                }
                ...
                if (gwInfo.nProtocolVersion < DGW_PROTO_VERSION) {
                    // gateway is not upgraded yet
                    continue;
                }
            }
        }
