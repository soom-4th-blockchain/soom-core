...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        FillBlockPayments(txNew, nHeight, blockReward, pblock->txoutGateway, pblock->txoutFoundation);
        // LogPrintf("CreateNewBlock -- nBlockHeight %d blockReward %lld txoutGateway %s txNew %s",
        //             nHeight, blockReward, pblock->txoutGateway.ToString(), txNew.ToString());
...
// ***TODO*** ScanHash is not yet used in Soom
//
...
{
    LogPrintf("SoomMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("soom-miner");
        ...
        while (true) {
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    if (!fvNodesEmpty && !IsInitialBlockDownload() && gatewaySync.IsSynced())
                        break;
                    MilliSleep(1000);
                } while (true);
            }
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("SoomMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("SoomMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("SoomMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("SoomMiner -- runtime error: %s\n", e.what());
        return;
    }
}
