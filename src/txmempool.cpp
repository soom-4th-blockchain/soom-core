...
bool CTxMemPool::addUnchecked(const uint256& hash, const CTxMemPoolEntry &entry, setEntries &setAncestors, bool validFeeEstimate)
{
    ...
    if (tx.nType == TRANSACTION_PROVIDER_REGISTER) {
        ...
        mapProTxRefs.emplace(proTx.proTxHash, tx.GetHash());
        mapProTxBlsPubKeyHashes.emplace(proTx.pubKeyOperator.GetHash(), tx.GetHash());
        auto dgw = deterministicGWManager->GetListAtChainTip().GetGW(proTx.proTxHash);
        assert(dgw);
        newit->validForProTxKey = ::SerializeHash(dgw->pdgwState->pubKeyOperator);
        if (dgw->pdgwState->pubKeyOperator != proTx.pubKeyOperator) {
        ...
        assert(dgw);
        newit->validForProTxKey = ::SerializeHash(dgw->pdgwState->pubKeyOperator);
        if (dgw->pdgwState->pubKeyOperator != CBLSPublicKey()) {
            newit->isKeyChangeProTx = true;
        }
    }
...
{
    // Remove TXs that refer to a GW for which the collateral was spent
    ...
    for (const auto& in : tx.vin) {
        ...
        auto dgw = gwList.GetGWByCollateral(in.prevout);
        if (dgw) {
            // These are updates refering to a mined ProRegTx
            removeSpentCollateralConflict(dgw->proTxHash);
        }
    }
}
...
bool CTxMemPool::existsProviderTxConflict(const CTransaction &tx) const {
    ...
    if (tx.nType == TRANSACTION_PROVIDER_REGISTER) {
        ...
        auto dgw = deterministicGWManager->GetListAtChainTip().GetGW(proTx.proTxHash);
        if (!dgw) {
            LogPrintf("%s: ERROR: Gateway is not in the list, proTxHash: %s", __func__, proTx.proTxHash.ToString());
...
CTxMemPool::WriteFeeEstimates(CAutoFile& fileout) const
{
    try {
        LOCK(cs);
        fileout << 1000300; // version required to read: 1.0.3.0 or later
