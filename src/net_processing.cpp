    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        LogPrint("net", "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_GATEWAY_PAYMENT_VOTE) {
                    if(gwpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << gwpayments.mapGatewayPaymentVotes[inv.hash];
                        connman.PushMessage(pfrom, NetMsgType::GATEWAYPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_GATEWAY_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                        ...
                        BOOST_FOREACH(CGatewayPayee& payee, gwpayments.mapGatewayBlocks[mi->second->nHeight].vecPayees) {
                            ...
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(gwpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << gwpayments.mapGatewayPaymentVotes[hash];
                                    connman.PushMessage(pfrom, NetMsgType::GATEWAYPAYMENTVOTE, ss);
                                }
                            }
                        }
                ...
                if (!pushed && inv.type == MSG_GATEWAY_ANNOUNCE) {
                    if(gwnodeman.mapSeenGatewayBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << gwnodeman.mapSeenGatewayBroadcast[inv.hash].second;
                        connman.PushMessage(pfrom, NetMsgType::GWANNOUNCE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_GATEWAY_PING) {
                    if(gwnodeman.mapSeenGatewayPing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << gwnodeman.mapSeenGatewayPing[inv.hash];
                        connman.PushMessage(pfrom, NetMsgType::GWPING, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_GATEWAY_VERIFY) {
                    if(gwnodeman.mapSeenGatewayVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << gwnodeman.mapSeenGatewayVerification[inv.hash];
                        connman.PushMessage(pfrom, NetMsgType::GWVERIFY, ss);
                        pushed = true;
                    }
                }
