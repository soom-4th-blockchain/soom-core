...
static CMutableTransaction CreateProRegTx(SimpleUTXOMap& utxos, int port, const CScript& scriptPayout, const CKey& coinbaseKey, CKey& ownerKeyRet, CBLSSecretKey& operatorKeyRet)
{
    ...
    CAmount change;
    auto inputs = SelectUTXOs(utxos, COLLATERAL_COINS, change);
    ...
    FundTransaction(tx, utxos, scriptPayout, COLLATERAL_COINS, coinbaseKey);
...
static CMutableTransaction CreateProUpRegTx(SimpleUTXOMap& utxos, const uint256& proTxHash, const CKey& gwKey, const CBLSPublicKey& pubKeyOperator, const CKeyID& keyIDVoting, const CScript& scriptPayout, const CKey& coinbaseKey)
{
    CAmount change;
    auto inputs = SelectUTXOs(utxos, 1 * COIN, change);
    ...
    FundTransaction(tx, utxos, GetScriptForDestination(coinbaseKey.GetPubKey().GetID()), 1 * COIN, coinbaseKey);
    proTx.inputsHash = CalcTxInputsHash(tx);
    CHashSigner::SignHash(::SerializeHash(proTx), gwKey, proTx.vchSig);
    SetTxPayload(tx, proTx);
    SignTransaction(tx, coinbaseKey);
...
static CDeterministicGWCPtr FindPayoutDgw(const CBlock& block)
{
    auto dgwList = deterministicGWManager->GetListAtChainTip();

    for (const auto& txout : block.vtx[0]->vout) {
        CDeterministicGWCPtr found;
        dgwList.ForEachGW(true, [&](const CDeterministicGWCPtr& dgw) {
            if (found == nullptr && txout.scriptPubKey == dgw->pdgwState->scriptPayout) {
                found = dgw;
            }
        });
        if (found != nullptr) {
...
BOOST_FIXTURE_TEST_CASE(dip3_activation, TestChainDIP3BeforeActivationSetup)
{
    ...
    BOOST_ASSERT(!deterministicGWManager->GetListAtChainTip().HasGW(tx.GetHash()));
    ...
    ProcessNewBlock(Params(), block, true, nullptr);
    deterministicGWManager->UpdatedBlockTip(chainActive.Tip());
...
BOOST_FIXTURE_TEST_CASE(dip3_protx, TestChainDIP3Setup)
{
    ...
    // activate spork15
    sporkManager.UpdateSpork(SPORK_15_DETERMINISTIC_GWS_ENABLED, chainActive.Height() + 1, *g_connman);
    CreateAndProcessBlock({}, coinbaseKey);
    deterministicGWManager->UpdatedBlockTip(chainActive.Tip());
    nHeight++;

    // check GW reward payments
    for (size_t i = 0; i < 20; i++) {
        auto dgwExpectedPayee = deterministicGWManager->GetListAtChainTip().GetGWPayee();

        CBlock block = CreateAndProcessBlock({}, coinbaseKey);
        deterministicGWManager->UpdatedBlockTip(chainActive.Tip());
        BOOST_ASSERT(!block.vtx.empty());

        auto dgwPayout = FindPayoutDgw(block);
        BOOST_ASSERT(dgwPayout != nullptr);
        BOOST_CHECK_EQUAL(dgwPayout->proTxHash.ToString(), dgwExpectedPayee->proTxHash.ToString());

        nHeight++;
    }

    // register multiple GWs per block
    for (size_t i = 0; i < 3; i++) {
        ...
        for (size_t j = 0; j < 3; j++) {
            BOOST_ASSERT(deterministicGWManager->GetListAtChainTip().HasGW(txns[j].GetHash()));
        }
    ...
    // test ProUpServTx
    auto tx = CreateProUpServTx(utxos, dgwHashes[0], operatorKeys[dgwHashes[0]], 1000, CScript(), coinbaseKey);
    CreateAndProcessBlock({tx}, coinbaseKey);
    deterministicGWManager->UpdatedBlockTip(chainActive.Tip());
    BOOST_ASSERT(chainActive.Height() == nHeight + 1);
    nHeight++;

    auto dgw = deterministicGWManager->GetListAtChainTip().GetGW(dgwHashes[0]);
    BOOST_ASSERT(dgw != nullptr && dgw->pdgwState->addr.GetPort() == 1000);

    // test ProUpRevTx
    tx = CreateProUpRevTx(utxos, dgwHashes[0], operatorKeys[dgwHashes[0]], coinbaseKey);
    CreateAndProcessBlock({tx}, coinbaseKey);
    deterministicGWManager->UpdatedBlockTip(chainActive.Tip());
    BOOST_ASSERT(chainActive.Height() == nHeight + 1);
    nHeight++;

    dgw = deterministicGWManager->GetListAtChainTip().GetGW(dgwHashes[0]);
    BOOST_ASSERT(dgw != nullptr && dgw->pdgwState->nPoSeBanHeight == nHeight);

    // test that the revoked GW does not get paid anymore
    for (size_t i = 0; i < 20; i++) {
        auto dgwExpectedPayee = deterministicGWManager->GetListAtChainTip().GetGWPayee();
        BOOST_ASSERT(dgwExpectedPayee->proTxHash != dgwHashes[0]);
    ...
    // test reviving the GW
    CBLSSecretKey newOperatorKey;
    newOperatorKey.MakeNewKey();
    dgw = deterministicGWManager->GetListAtChainTip().GetGW(dgwHashes[0]);
    tx = CreateProUpRegTx(utxos, dgwHashes[0], ownerKeys[dgwHashes[0]], newOperatorKey.GetPublicKey(), ownerKeys[dgwHashes[0]].GetPubKey().GetID(), dgw->pdgwState->scriptPayout, coinbaseKey);
    CreateAndProcessBlock({tx}, coinbaseKey);
    deterministicGWManager->UpdatedBlockTip(chainActive.Tip());
    BOOST_ASSERT(chainActive.Height() == nHeight + 1);
    nHeight++;

    tx = CreateProUpServTx(utxos, dgwHashes[0], newOperatorKey, 100, CScript(), coinbaseKey);
    CreateAndProcessBlock({tx}, coinbaseKey);
    deterministicGWManager->UpdatedBlockTip(chainActive.Tip());
    BOOST_ASSERT(chainActive.Height() == nHeight + 1);
    nHeight++;
    ...
    dgw = deterministicGWManager->GetListAtChainTip().GetGW(dgwHashes[0]);
    BOOST_ASSERT(dgw != nullptr && dgw->pdgwState->addr.GetPort() == 100);
    BOOST_ASSERT(dgw != nullptr && dgw->pdgwState->nPoSeBanHeight == -1);

    // test that the revived GW gets payments again
    bool foundRevived = false;
    for (size_t i = 0; i < 20; i++) {
        auto dgwExpectedPayee = deterministicGWManager->GetListAtChainTip().GetGWPayee();
        if (dgwExpectedPayee->proTxHash == dgwHashes[0]) {
            foundRevived = true;
        }
