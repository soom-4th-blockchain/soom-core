...
#define BOOST_TEST_MODULE Soom Test Suite
...
TestingSetup::TestingSetup(const std::string& chainName) : BasicTestingSetup(chainName)
{
    const CChainParams& chainparams = Params();
        ...
        ClearDatadirCache();
        pathTemp = GetTempPath() / strprintf("test_soom_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(100000)));
...
CBlock TestChainSetup::CreateBlock(const std::vector<CMutableTransaction>& txns, const CScript& scriptPubKey)
{
    ...
    std::vector<CTransactionRef> llgqCommitments;
    for (const auto& tx : block.vtx) {
        if (tx->nVersion == 3 && tx->nType == TRANSACTION_QUORUM_COMMITMENT) {
            llgqCommitments.emplace_back(tx);
        }
    }
    ...
    block.vtx.insert(block.vtx.end(), llgqCommitments.begin(), llgqCommitments.end());
    ...
    if (block.vtx[0]->nType == TRANSACTION_COINBASE) {
        ...
        CValidationState state;
        if (!CalcCbTxMerkleRootGWList(block, chainActive.Tip(), cbTx.merkleRootGWList, state)) {
            BOOST_ASSERT(false);
        }
