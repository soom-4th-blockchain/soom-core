    ...
    pollTimer(0)
{
    ...
    pollGwTimer = new QTimer(this);
    connect(pollGwTimer, SIGNAL(timeout()), this, SLOT(updateGwTimer()));
...
QString ClientModel::getGatewayCountString() const
{
    ...
    return tr("Total: %1 (Enabled: %2)")
            ...
            // .arg(QString::number((int)gwnodeman.CountByIP(NET_IPV4)))
            // .arg(QString::number((int)gwnodeman.CountByIP(NET_IPV6)))
            // .arg(QString::number((int)gwnodeman.CountByIP(NET_TOR)));
}
...
void ClientModel::updateGwTimer()
{
    QString newGatewayCountString = getGatewayCountString();

    if (cachedGatewayCountString != newGatewayCountString)
    {
        cachedGatewayCountString = newGatewayCountString;

        Q_EMIT strGatewaysChanged(cachedGatewayCountString);
    }
}
