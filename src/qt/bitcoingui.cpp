    ...
    platformStyle(platformStyle)
{
    ...
    QString windowTitle = tr("Soom Core") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Soom address"));
...
#ifdef Q_OS_MAC
    signMessageAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_5));
#else
    signMessageAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
#endif
...
#ifdef ENABLE_WALLET
    QSettings settings;
    if (settings.value("fShowGatewaysTab").toBool()) {
        gatewayAction = new QAction(QIcon(":/icons/" + theme + "/gateways"), tr("&Gateways"), this);
        gatewayAction->setStatusTip(tr("Browse gateways"));
        gatewayAction->setToolTip(gatewayAction->statusTip());
        gatewayAction->setCheckable(true);
#ifdef Q_OS_MAC
        gatewayAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_9));
#else
        gatewayAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
#endif
        tabGroup->addAction(gatewayAction);
        connect(gatewayAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
        connect(gatewayAction, SIGNAL(triggered()), this, SLOT(gotoGatewayPage()));
    }
...
//    connect(usedReceivingAddressesMenuAction, SIGNAL(triggered()), this, SLOT(gotoUsedReceivingAddressesPage()));
    ...
    aboutAction = new QAction(/*QIcon(":/icons/" + theme + "/about"), */tr("&About Soom Core"), this);
    aboutAction->setStatusTip(tr("Show information about Soom Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Soom Core"));
...
//    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Soom addresses"));
    ...
    openAction->setStatusTip(tr("Open a soom: URI or payment request"));

    showHelpMessageAction = new QAction(/*QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), */tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Soom Core help message to get a list with possible Soom Core command-line options"));
    ...
    connect(openGWConfEditorAction, SIGNAL(triggered()), this, SLOT(showGWConfEditor()));
...
//        tools->addAction(openGWConfEditorAction);
...
#ifdef _DOCKWIDGET
        Qt::WindowFlags flags = Qt::CustomizeWindowHint | Qt::BypassGraphicsProxyWidget | Qt::NoDropShadowWindowHint;
        QDockWidget *dock = new QDockWidget(tr(""), this, flags);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    if (settings.value("fShowGatewaysTab").toBool() && gatewayAction) {
        gatewayAction->setEnabled(enabled);
    }
...
//    usedReceivingAddressesMenuAction->setEnabled(enabled);
...
void BitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("Soom Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::showGWConfEditor()
{
    GUIUtil::openGWConfigfile();
}
...
void BitcoinGUI::gotoGatewayPage()
{
    QSettings settings;
    if (settings.value("fShowGatewaysTab").toBool()) {
...
void BitcoinGUI::gotoUsedSendingAddressesPage()
{
    usedSendingAddressesAction->setChecked(true);
    if (walletFrame) walletFrame->gotoUsedSendingAddressesPage();
}

void BitcoinGUI::gotoUsedReceivingAddressesPage()
{
    usedReceivingAddressesAction->setChecked(true);
    if (walletFrame) walletFrame->gotoUsedReceivingAddressesPage();
}
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Soom network", "", count));
    } else {
...
void BitcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in bitcoin
        {
            ...
            // TODO instead of hiding it forever, we should add meaningful information about GW sync to the overlay
    ...
    if(!gatewaySync.IsBlockchainSynced())
    {
...
void BitcoinGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    ...
    strSyncStatus = QString(gatewaySync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("Soom Core"); // default title
