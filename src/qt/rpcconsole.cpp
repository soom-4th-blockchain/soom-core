...
void RPCConsole::setClientModel(ClientModel *model)
{
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        setGatewayCount(model->getGatewayCountString());
        connect(model, SIGNAL(strGatewaysChanged(QString)), this, SLOT(setGatewayCount(QString)));
...
void RPCConsole::clear(bool clearHistory)
{
    ...
    QString errColor = GUIUtil::getThemeName() == "light" ? "red" : "#E85552";
    ui->messagesWidget->document()->setDefaultStyleSheet(
        QString(
                "table { }"
                "td.time { color:#808080; font-size: %2; padding-top:3px; } "
                "td.message { font-family: %1; font-size: %2; white-space:pre-wrap; } "
                "td.cmd-request { color:#1E8989; } "
                "td.cmd-error { color: %3; } "
                ".secwarning { color: %3; font-weight:bold; }"
                "b { color:#1E8989; } "
            ).arg(fixedFontInfo.family(), QString("%1pt").arg(consoleFontSize), errColor)
        );
...
void RPCConsole::setGatewayCount(const QString &strGateways)
{
    ui->gatewayCount->setText(strGateways);
}
