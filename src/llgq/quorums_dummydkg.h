...
#ifndef SOOM_QUORUMS_DUMMYDKG_H
#define SOOM_QUORUMS_DUMMYDKG_H
 ...
 *
 * The purpose of this dummy implementation is to test final LLGQ commitments and simple PoSe on-chain.
 * The dummy DKG first creates dummy commitments and propagates these to all nodes. They can then create
 * a valid LLGQ commitment from these, which validates with the normal commitment validation code.
 *
...
private:
    ...
    std::map<Consensus::LLGQType, CDummyDKGSession> curSessions;
...
public:
    ...
    void CreateDummyContribution(Consensus::LLGQType llgqType, const CBlockIndex* pindex);
    void CreateDummyCommitment(Consensus::LLGQType llgqType, const CBlockIndex* pindex);
    void CreateFinalCommitment(Consensus::LLGQType llgqType, const CBlockIndex* pindex);
...
private:
    std::vector<bool> GetValidMembers(Consensus::LLGQType llgqType, const std::vector<CDeterministicGWCPtr>& members);
    BLSSecretKeyVector BuildDeterministicSvec(Consensus::LLGQType llgqType, const uint256& quorumHash);
