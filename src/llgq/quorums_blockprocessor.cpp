    ...
    if (strCommand == NetMsgType::QFCOMMITMENT) {
        ...
        if (!qc.Verify(members, true)) {
            ...
            LogPrintf("CQuorumBlockProcessor::%s -- commitment for quorum %s:%d is not valid, peer=%d\n", __func__,
                      qc.quorumHash.ToString(), qc.llgqType, pfrom->id);
            Misbehaving(pfrom->id, 100);
            return;
        }

        LogPrintf("CQuorumBlockProcessor::%s -- received commitment for quorum %s:%d, validMembers=%d, signers=%d, peer=%d\n", __func__,
                  qc.quorumHash.ToString(), qc.llgqType, qc.CountValidMembers(), qc.CountSigners(), pfrom->id);
...
bool CQuorumBlockProcessor::ProcessBlock(const CBlock& block, const CBlockIndex* pindex, CValidationState& state)
{
    ...
    std::map<Consensus::LLGQType, CFinalCommitment> qcs;
...
{
    auto& params = Params().GetConsensus().llgqs.at((Consensus::LLGQType)qc.llgqType);

    uint256 quorumHash = GetQuorumBlockHash((Consensus::LLGQType)qc.llgqType, pindex->nHeight);
    ...
    auto members = CLLGQUtils::GetAllQuorumMembers(params.type, quorumHash);
    ...
    LogPrintf("CQuorumBlockProcessor::%s -- processed commitment from block. type=%d, quorumHash=%s, signers=%s, validMembers=%d, quorumPublicKey=%s\n", __func__,
              qc.llgqType, quorumHash.ToString(), qc.CountSigners(), qc.CountValidMembers(), qc.quorumPublicKey.ToString());
...
bool CQuorumBlockProcessor::UndoBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    for (auto& p : qcs) {
        ...
        evoDb.Erase(std::make_pair(DB_MINED_COMMITMENT, std::make_pair(qc.llgqType, qc.quorumHash)));
...
bool CQuorumBlockProcessor::GetCommitmentsFromBlock(const CBlock& block, const CBlockIndex* pindexPrev, std::map<Consensus::LLGQType, CFinalCommitment>& ret, CValidationState& state)
{
    AssertLockHeld(cs_main);
    ...
    for (const auto& tx : block.vtx) {
        if (tx->nType == TRANSACTION_QUORUM_COMMITMENT) {
            ...
            if (ret.count((Consensus::LLGQType)qc.commitment.llgqType)) {
                return state.DoS(100, false, REJECT_INVALID, "bad-qc-dup");
            }

            ret.emplace((Consensus::LLGQType)qc.commitment.llgqType, std::move(qc.commitment));
        }
    }
...
bool CQuorumBlockProcessor::IsMiningPhase(Consensus::LLGQType llgqType, int nHeight)
{
...
bool CQuorumBlockProcessor::IsCommitmentRequired(Consensus::LLGQType llgqType, int nHeight)
{
    uint256 quorumHash = GetQuorumBlockHash(llgqType, nHeight);
    ...
    bool isMiningPhase = !quorumHash.IsNull() && IsMiningPhase(llgqType, nHeight);

    // did we already mine a non-null commitment for this session?
    bool hasMinedCommitment = !quorumHash.IsNull() && HasMinedCommitment(llgqType, quorumHash);
...
uint256 CQuorumBlockProcessor::GetQuorumBlockHash(Consensus::LLGQType llgqType, int nHeight)
{
    AssertLockHeld(cs_main);
...
bool CQuorumBlockProcessor::HasMinedCommitment(Consensus::LLGQType llgqType, const uint256& quorumHash)
{
    auto key = std::make_pair(DB_MINED_COMMITMENT, std::make_pair((uint8_t)llgqType, quorumHash));
    return evoDb.Exists(key);
}

bool CQuorumBlockProcessor::GetMinedCommitment(Consensus::LLGQType llgqType, const uint256& quorumHash, CFinalCommitment& ret)
{
    auto key = std::make_pair(DB_MINED_COMMITMENT, std::make_pair((uint8_t)llgqType, quorumHash));
...
void CQuorumBlockProcessor::AddMinableCommitment(const CFinalCommitment& fqc)
{
...

    {
        ...
        auto k = std::make_pair((Consensus::LLGQType) fqc.llgqType, fqc.quorumHash);
...
bool CQuorumBlockProcessor::GetMinableCommitmentByHash(const uint256& commitmentHash, llgq::CFinalCommitment& ret)
{
    LOCK(minableCommitmentsCs);
...
bool CQuorumBlockProcessor::GetMinableCommitment(Consensus::LLGQType llgqType, int nHeight, CFinalCommitment& ret)
{
    AssertLockHeld(cs_main);

    if (!IsCommitmentRequired(llgqType, nHeight)) {
    ...
    auto k = std::make_pair(llgqType, quorumHash);
    auto it = minableCommitmentsByQuorum.find(k);
    if (it == minableCommitmentsByQuorum.end()) {
        // null commitment required
        ret = CFinalCommitment(Params().GetConsensus().llgqs.at(llgqType), quorumHash);
        return true;
    }
...
bool CQuorumBlockProcessor::GetMinableCommitmentTx(Consensus::LLGQType llgqType, int nHeight, CTransactionRef& ret)
{
    AssertLockHeld(cs_main);

    CFinalCommitmentTxPayload qc;
    if (!GetMinableCommitment(llgqType, nHeight, qc.commitment)) {
        return false;
    }
